...
class RecoveryAgent(BaseAgent):
    ...
    async def _propose(self, session, event: AgentEvent) -> None:
        import uuid
        from app.agents.schemas import RecoveryPlan as RecoverySchema
        ...
        await self.think(
            session.id,
            f"Training anomaly: {anomaly}. Generating a recovery plan.",
            parent=event.id,
        )

        # Try schema-validated LLM first.
        proposal = await self.call_llm_typed(
            session.id,
            (
                f"Anomaly: {anomaly}\nConfig: {cfg}\n\n"
                "Propose a recovery plan. Use op 'set' for config edits, "
                "'noop' for retry-only, 'stop' to escalate. Each op has "
                "(op, path, new). Return RecoveryPlan JSON."
            ),
            RecoverySchema,
            system="You are a training recovery specialist. Output only valid JSON.",
            stream_thoughts=False,
            parent=event.id,
        )
        if proposal and proposal.operations:
            plan = proposal.model_dump()
            plan.setdefault("diff_id", str(uuid.uuid4()))
        else:
        ...
        if "error" in plan or not plan.get("operations"):
            await self.emit_message(
                session.id,
                f"Recovery: no automatic fix for {anomaly}.",
                parent=event.id,
            )
            return
